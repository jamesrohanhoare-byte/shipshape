...
> Last updated: 2026-06-02 · Current version: **v1.14.0** (live).
> Recent: v1.8 currency + financial-toggle (00009) · v1.9 settings-reset fix + Home-centred nav · v1.10 notifications (00010 shopping-dismiss; notify-task + daily-task-digest fns; pg_cron digest, CRON_SECRET) · v1.10.1 segmented-control centring fix · v1.11 Crew Hours + MLC compliance (00011 time_logs) · v1.12 Reports Used/Bought toggle · v1.13 Home command-centre + Tasks done-hint + Settings grouping · v1.14 visual onboarding rewrite. Migrations now 00001–00011. Edge fns: notify-low-stock, create-crew-member, notify-task, daily-task-digest. Deploy NOTE: Vercel now AUTO-deploys on push to main (stage on a branch until backend is ready). Notifications deploy guide: docs/notifications-deploy.md.
...
- `items` — id, boat_id, name, category_id, unit_id, price_per_unit, par_level, **current_quantity** (cached), location (stored-on-boat), **purchase_location (where bought, v1.7.0)**, photo_url.
...
- **v1.7.0 — quick wins.** (A) **Stock bulk delete** — Select mode on the Stock page: per-row checkboxes, select-all/deselect-all, delete selected (captain/manager, RLS-gated). (B) **Item `purchase_location`** ("Bought at") — migration `00008`; shown on the item form next to storage Location; **shopping list groups by store** (Share text too). (C) **PWA app icons** — real PNG set (192/512/maskable + 180 apple-touch) generated from `public/icon.svg` via `npm run icons` (`scripts/generate-icons.mjs`, uses `sharp` devDep); manifest + `apple-touch-icon` now point to PNG (SVG-only was why the home-screen icon was blank). Migrations now `00001–00008`.
